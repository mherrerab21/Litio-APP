...
if option == 'Precios de Contrato':
    ...
    # Load Excel File
    nombre_archivo_excel = 'futuros litio.xlsx'
    book = load_workbook(nombre_archivo_excel, data_only=True)
    hoja_contrato2407 = book['Contrato 2407']

    # Extract Data from Excel
    data = []
    for row in hoja_contrato2407.iter_rows(values_only=True):
        data.append(row)

    column_headers = data[0]
    df_lc2407 = pd.DataFrame(data[1:], columns=column_headers)
